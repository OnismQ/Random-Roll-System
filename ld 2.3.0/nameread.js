...
const systemReduceMotionQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
...
async function refreshWhenVersionChanges() {
    if (!window.location.protocol.startsWith("http")) {
        return;
    }

    const currentVersion = document.querySelector('meta[name="lucky-dog-version"]')?.content;
    if (!currentVersion) {
        return;
    }
    ...
    try {
        ...
        const latestVersion = latestDocument.querySelector('meta[name="lucky-dog-version"]')?.content;
        if (!latestVersion || latestVersion === currentVersion) {
            return;
        }

        const refreshUrl = new URL(window.location.href);
        refreshUrl.searchParams.set("ld-update", Date.now().toString(36));
...
window.setTimeout(refreshWhenVersionChanges, 1200);
...
function finishActionDialog(result) {
    ...
    actionDialogCloseTimer = setTimeout(finalizeActionDialogClose, motionDelay(180));
}
...
function applyReadability(enabled, persist = true) {
    const shouldImproveReadability = Boolean(enabled);
    document.documentElement.dataset.readability = String(shouldImproveReadability);
    readabilityToggle.checked = shouldImproveReadability;

    if (persist) {
        writeStorage("luckyDogReadability", String(shouldImproveReadability));
    }
}

function applyReduceMotion(enabled, persist = true) {
    const shouldReduceMotion = Boolean(enabled);
    document.documentElement.dataset.reduceMotion = String(shouldReduceMotion);
    reduceMotionToggle.checked = shouldReduceMotion;

    if (persist) {
        writeStorage("luckyDogReduceMotion", String(shouldReduceMotion));
    }

    window.dispatchEvent(new CustomEvent("lucky-dog:motion-change", {
        detail: { reduceMotion: shouldReduceMotion }
    }));
}

const storedReadabilityPreference = readStorage("luckyDogReadability");
const legacyReadabilityPreference =
    readStorage("luckyDogHighContrast") === "true" ||
    readStorage("luckyDogLargeText") === "true";

applyReadability(
    storedReadabilityPreference === null
        ? legacyReadabilityPreference
        : storedReadabilityPreference === "true",
    false
);
applyReduceMotion(reduceMotionEnabled(), false);

readabilityToggle.addEventListener("change", () => {
    applyReadability(readabilityToggle.checked);
});
reduceMotionToggle.addEventListener("change", () => {
    applyReduceMotion(reduceMotionToggle.checked);
});

systemReduceMotionQuery.addEventListener("change", event => {
    if (readStorage("luckyDogReduceMotion") === null) {
        applyReduceMotion(event.matches, false);
    }
});
...
function closeSettingsModal() {
    ...
    settingsCloseTimer = setTimeout(finalizeSettingsClose, motionDelay(180));
    return settingsClosePromise;
}
...
let reducedMotionPendingResult = false;
...
async function startRandom() {
    ...
    nameDisplay.setAttribute("aria-busy", "true");

    if (reduceMotionEnabled()) {
        reducedMotionPendingResult = true;
        nameDisplay.textContent = "抽取中…";
    } else {
        nameDisplay.classList.add("is-running");
...
function pauseRandom() {
    const wasRunning = interval !== null || autoPauseTimeout !== null || reducedMotionPendingResult;
    ...
    if (reducedMotionPendingResult) {
        nameDisplay.textContent = getRandomName();
        reducedMotionPendingResult = false;
    }

    nameDisplay.classList.remove("is-running");
    nameDisplay.setAttribute("aria-busy", "false");
    startBtn.disabled = false;
    pauseBtn.disabled = true;

    if (wasRunning) {
        resultAnnouncement.textContent = `点名结果：${nameDisplay.textContent}`;
    }
}

window.addEventListener("lucky-dog:motion-change", event => {
    const shouldReduceMotion = Boolean(event.detail?.reduceMotion);
    if (shouldReduceMotion && interval !== null) {
        clearInterval(interval);
        interval = null;
        reducedMotionPendingResult = true;
        nameDisplay.classList.remove("is-running");
        nameDisplay.textContent = "抽取中…";
    } else if (!shouldReduceMotion && reducedMotionPendingResult && autoPauseTimeout !== null) {
        reducedMotionPendingResult = false;
        nameDisplay.classList.add("is-running");
        nameDisplay.textContent = getRandomName();
        interval = setInterval(() => {
